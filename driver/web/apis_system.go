...
func (h SystemAPIsHandler) getExample(l *logs.Log, r *http.Request, claims *tokenauth.Claims) logs.HTTPResponse {
	params := mux.Vars(r)
	id := params["id"]
	if len(id) <= 0 {
	...
	example, err := h.app.System.GetExample(claims.OrgID, claims.AppID, id)
	if err != nil {
